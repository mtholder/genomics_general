...
def raxTree(
    ...
    # temp file
    tempAln = tempfile.NamedTemporaryFile(
        mode="w", prefix=prefix, suffix=".phy", dir=".", delete=False
    )
...
parser.add_argument(
    "--windType",
    help="Type of windows to make",
    action="store",
    choices=("sites", "coordinate"),
    default="coordinate",
)
...
parser.add_argument(
    "-M",
    "--minSites",
    help="Minumum good sites per window",
    type=int,
    action="store",
    required=False,
    metavar="sites",
...
parser.add_argument(
    "-Mi",
    "--minPerInd",
    help="Minumum good sites per individual",
    type=int,
    action="store",
    required=False,
    metavar="sites",
...
parser.add_argument(
    "-S",
    "--stepSize",
    help="Step size for coordinate sliding window",
    type=int,
    action="store",
    required=False,
    metavar="sites",
...
parser.add_argument(
    "-O",
    "--overlap",
    help="Overlap for sites sliding window",
    type=int,
    action="store",
    required=False,
    metavar="sites",
...
parser.add_argument(
    "-D",
    "--maxDist",
    help="Maximum span distance for sites window",
    type=int,
    action="store",
    required=False,
)
...
parser.add_argument(
    "--individuals",
    help="Individuals to include, separated by comma",
    action="store",
    metavar="ind1,ind2,ind3...",
)

parser.add_argument(
    "--outgroup",
    help="Outgroup individuals, separated by comma",
    action="store",
    metavar="ind1,ind2,ind3...",
)
...
parser.add_argument(
    "--raxml",
    help="path to raxml executable",
    action="store",
    metavar="path/to/raxml",
    default="raxml",
)
...
parser.add_argument(
    "--log",
    help="raxml log file, if you want one.",
    action="store",
    metavar="path/to/log",
    default="/dev/null",
)
...
if args.windType == "coordinate":
    ...
    assert (
        not args.overlap
    ), "Overlap noes not apply to coordinate windows. Use --stepSize instead."
    ...
    assert (
        not args.stepSize
    ), "Step size only applies to coordinate windows. Use --overlap instead."
...
if args.individuals:
    indNames = args.individuals.split(",")
else:
    indNames = None
...
if args.outgroup:
    ...
    if test or verbose:
        print("outgroups:", " ".join(outgroup), file=sys.stderr)
...
if args.genoFile:
    genoFile = (
        gzip.open(args.genoFile, "r")
        if args.genoFile.endswith(".gz")
        else open(args.genoFile, "r")
    )
else:
    genoFile = sys.stdin
...
for window in windowGenerator:
    ...
    while windowsQueued - resultsReceived >= 50:
        sleep(10)
        if test or verbose:
            print("Waiting for queue to clear...", file=sys.stderr)
