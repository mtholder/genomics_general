...
def haplo(diplo):
    return diploHaploDict[diplo]


def diplo(pair):
    return haploDiploDict[pair]


def homo(diplo):
    return diploHomoDict[diplo]
...
def complement(seq):
    if type(seq) == str:
        return seq.translate(complementTrans)
    else:
        return [complementDict[a] for a in seq]
...
def revComplement(seq):
    if type(seq) == str:
        ...
        return [complementDict[a] for a in seq[::-1]]


def allPossibleSeqs(seq, ignoreNs=True):
    if ignoreNs:
        basesList = [iupacBasesDict[s] if s != "N" else "N" for s in seq]
    else:
        basesList = [iupacBasesDict[s] for s in seq]
    seqs = [[]]
...
def seqArrayToNumArray(seqArray):
    ...
    for x in ["A", "C", "G", "T", "N"]:
        numArray[seqArray == x] = seqNumDict[x]
    return numArray


def numArrayToSeqArray(numArray):
    seqArray = np.empty(shape=numArray.shape, dtype=str)
    for x in [0, 1, 2, 3, -999]:
        seqArray[numArray == x] = numSeqDict[x]
    return seqArray
...
def nanmean_min(a, min=0):
    if 1 - (1.0 * np.isnan(a).sum() / a.size) < min:
        return np.NaN
    return np.nanmean(a)
...
def translate(sequence, missing="X"):
    ...
    return "".join(
        [
            gencode.get(sequence[3 * i : 3 * i + 3], missing)
...
def possibleCodons(pos1Alleles, pos2Alleles, pos3Alleles):
    return [
        "".join(x) for x in itertools.product(pos1Alleles, pos2Alleles, pos3Alleles)
    ]
...
def possibleAAs(pos1Alleles, pos2Alleles, pos3Alleles):
    AAs = set(
        [
            translate(codon)
            for codon in possibleCodons(pos1Alleles, pos2Alleles, pos3Alleles)
        ]
    )
    try:
        AAs.remove("X")
...
def degeneracy(pos1Alleles, pos2Alleles, pos3Alleles):
    ...
    if len(pos1Alleles) == len(pos2Alleles) == len(pos3Alleles) == 1:
        output = [
            degenDict[len(possibleAAs("ACGT", pos2Alleles, pos3Alleles))],
...
def parseGenes(lines, fmt="gff3", targets=None):
    ...
    if fmt == "gtf":
        makeInfoDict = lambda infoString: dict(
            [x.strip().split() for x in infoString.strip(";").split(";")]
        )
    ...
    for line in lines:
        if len(line) > 1 and line[0] != "#":
            ...
            if (
                gffObjects[2] == "mRNA"
                or gffObjects[2] == "mrna"
                or gffObjects[2] == "MRNA"
                or gffObjects[2] == "transcript"
            ):
                # we've found a new mRNA
                try:
                    mRNA = makeInfoDict(gffObjects[-1])[ID_key]
                except:
                    raise ValueError(
                        "Problem parsing mRNA information: " + gffObjects[-1]
                    )
                if not targets or mRNA in targets:
                    output[scaffold][mRNA] = {
...
def CDSpositions(exonStarts, exonEnds, strand, trim=False):
    ...
    if strand == "-":
        for _codingPositions_ in codingPositions:
            _codingPositions_.reverse()

    # unlist them to make one long set of positions
    codingPositions = [
        i for _codingPositions_ in codingPositions for i in _codingPositions_
    ]

    if trim:
        overhang = len(codingPositions) % 3
        if overhang != 0:
            codingPositions = codingPositions[:-overhang]
...
# function to extract a CDS sequence from a genomic sequence given the exon starts, ands and strand
def CDSsequence(
    exonStarts, exonEnds, strand, seqDict=None, seq=None, seqPos=None, trim=True
):
    ...
    if strand == "-":
        cdsSeq = cdsSeq.translate(complementTrans)
...
def chunkList(l, nChunks=None, chunkSize=None, return_indices=False):
    ...
    if return_indices:
        return (
            [[l[x] for x in ind] for ind in indices],
            indices,
        )
    else:
        return [[l[x] for x in ind] for ind in indices]
...
class Genotype:
    ...
    def __init__(
        self, geno, genoFormat, ploidy=None, forcePloidy=False, partialToMissing=False
    ):
        if genoFormat == "phased":
            ...
            raise ValueError(
                "Valid genotype formats are 'phased' (eg A/T), 'alleles' (eg AT), 'pairs' (eg AT), 'haplo' (eg A) or 'diplo' (eg W)"
            )
        ...
        if ploidy is not None:
            ...
            if ploidyError != 0:
                if forcePloidy:
                    if ploidyError > 0:
                        self.alleles += ["N"] * ploidyError
                    elif ploidyError < 0:
                        if len(set(self.alleles)) == 1:
                            self.alleles = [self.alleles[0]] * ploidy
                        else:
                            self.alleles = ["N"] * ploidy
                else:
                    raise ValueError("Ploidy doesn't match number of alleles")
        else:
            ploidy = len(self.alleles)

        if partialToMissing and "N" in self.alleles:
            self.alleles = ["N"] * ploidy
        ...
        try:
            self.numAlleles = np.array([seqNumDict[a] for a in self.alleles])
        except:
            self.numAlleles = np.array([-999] * self.ploidy)

    def isHaploid(self):
        return self.ploidy == 1

    def asPhased(self):
        return self.phase.join(self.alleles)
    ...
    def asCoded(
        self, codeDict, missing=None
    ):  # code alleles e.g. 0 and 1, with phase (0/1)
        if missing is None:
            missing = "."
        try:
            return self.phase.join([codeDict[a] for a in self.alleles])
        except:
            return self.phase.join([missing] * self.ploidy)

    def asCount(self, countAllele, missing=None):  # code whole genotype as single value
        if missing is None:
            missing = -1
        try:
            return np.bincount(self.numAlleles, minlength=4)[seqNumDict[countAllele]]
        except:
            return missing
    ...
    def asRandomAllele(self):
        return (
            ...
            else random.sample(self.alleles, 1)[0]
        )

    def isMissing(self):
        return np.any(self.numAlleles == -999)
...
def pseudoPhase(sequence, genoFormat="diplo"):
    if genoFormat == "pairs":
        return [[g[0] for g in sequence], [g[1] for g in sequence]]
    elif genoFormat == "phased":
        return [[g[0] for g in sequence], [g[2] for g in sequence]]
    else:
...
def splitSeq(sequence, genoFormat="phased"):
    ...
    if genoFormat == "diplo":
        sequence = [haplo(d) for d in sequence]
...
def haploToPhased(seqs, seqNames=None, ploidy=2, randomPhase=False):
    ...
    if set(_ploidy_) == {1}:
        ...
        if len(_ploidy_) == 1:
            assert (
                Nseqs % _ploidy_[0] == 0
            ), "Sequence number must be divizable by ploidy"
...
def makeHaploidNames(names, ploidy=2):
    ...
    return [
        n + "_" + letter
        for n in names
        for letter in string.ascii_uppercase[: ploidyDict[n]]
    ]
...
class GenomeSite:
    def __init__(
        ...
        if not genoDict:
            ...
            ), "Either a genotypes dictionary or list must be specified."
            if not sampleNames:
                sampleNames = [str(x) for x in range(len(genotypes))]
            assert len(genotypes) == len(
                sampleNames
        ...
        for sample in self.sampleNames:
            ...
            # if not, compute gentype normally
                ...
                self.genotypes[sample] = Genotype(
                    ...
                    ploidy=self.ploidy[sample],
                    forcePloidy=forcePloidy,
                    partialToMissing=partialToMissing,
                )
    ...
    def asList(
        ...
        if mode == "bases":
            ...
            if alleleOrder == "freq":
                ...
                return [
                    ...
                    for a in sorted(
                        self.genotypes[sample].alleles,
                        key=lambda x: siteAlleles.index(x),
                    )
                ]
            # otherwise just return the bases as they appear
                ...
                return [
                    a
                    for alleles in [
                        self.genotypes[sample].alleles for sample in samples
                    ]
                    for a in alleles
                ]
        ...
        if mode == "numeric":
            return np.concatenate(
                [self.genotypes[sample].numAlleles for sample in samples]
            )
            ...
            mode == "coded"
        ):  # vcf format '0/1' - optionally alleles can be provided (REF first)
            if alleles is None:
            ...
            return [
                self.genotypes[sample].asCount(countAllele, missing)
                for sample in samples
            ]
        else:
            raise ValueError(
                "mode must be 'bases', 'alleles', 'numeric', 'numAlleles', 'phased', 'diplo', 'randomAllele', 'coded', or 'count'"
            )
    ...
    def alleles(self, samples=None, pop=None, byFreq=False, numeric=False):
        ...
        counts = counts[idx]
        if byFreq:
            return list(alleles[np.argsort(counts)[::-1]])
        else:
    ...
    def hets(self, samples=None):
        ...
        nSampAlleles = np.array(
            [len(uniqueAlleles) for uniqueAlleles in sampUniqueAlleles]
        )
        return 1.0 * (nSampAlleles > 1).sum() / self.nonMissing()

    def nonMissing(self, prop=False):
        present = sum(
            [~self.genotypes[sample].isMissing() for sample in self.sampleNames]
        )
        if prop:
            return 1.0 * present / (len(sampleNames))
        else:
            return present
...
def binBaseFreqs(numArr, asCounts=False):
    ...
    if n == 0:
        ...
        if asCounts:
            return np.bincount(numArr, minlength=4)
        else:
            return 1.0 * np.bincount(numArr, minlength=4) / n
...
def derivedAllele(
    ...
    inAlleles=[],
    outAlleles=[],
    maxOneDerivedAllele=True,
    numeric=False,
):

    if inBases is not None:
        inAlleles = np.unique(inBases)
    elif inBaseCounts is not None:
        if not numeric:
            inAlleles = ["ACGT"[i] for i in range(4) if inBaseCounts[i] > 0]
        else:
            inAlleles = np.where(np.array(inBaseCounts) > 0)[0]

    if outBases is not None:
        outAlleles = np.unique(outBases)
    elif outBaseCounts is not None:
        if not numeric:
            outAlleles = ["ACGT"[i] for i in range(4) if outBaseCounts[i] > 0]
        outAlleles = np.where(np.array(outBaseCounts) > 0)[0]

    if not isinstance(inAlleles, np.ndarray):
        inAlleles = np.array(inAlleles)
    if not isinstance(outAlleles, np.ndarray):
        outAlleles = np.array(outAlleles)

    if (
        maxOneDerivedAllele
        and len(outAlleles) == 1
        and len(inAlleles) == 2
        and np.any(outAlleles[0] == inAlleles)
    ):
        return inAlleles[inAlleles != outAlleles[0]][0]

    elif (
        not maxOneDerivedAllele
        and len(outAlleles) == 1
        and len(inAlleles) >= 2
        and np.any(outAlleles[0] == inAlleles)
    ):
        ...
        numeric
        or isinstance(inAlleles[0], np.int)
        or isinstance(inAlleles[0], np.float)
    ):
        return np.nan
    else:
        return "N"
...
def HWEtest(obsHet, obsHom1, obsHom2, side="both"):
    ...
    while currHet >= 2:
        probs[currHet - 2] = (
            ...
            * currHet
            * (currHet - 1.0)
            / (4.0 * (currHomRare + 1.0) * (currHomCom + 1.0))
        )
    ...
    if side == "top":
        p = min(1.0, sum(probs[obsHet : (rare + 1)]) / mySum)
    elif side == "bottom":
        p = min(1.0, sum(probs[0 : (obsHet + 1)]) / mySum)
    else:
        target = probs[obsHet]
...
def inHWE(diplos, P_value, side="both", verbose=False):
    ...
    if verbose:
        sys.stderr.write(str(Hom1Count) + " " + str(Hom2Count) + " " + str(HetCount))
...
def siteTest(
    ...
    HWE_side="both",
    fixed=False,
    nearlyFixedDiff=None,
):
    if not samples:
        samples = site.sampleNames
    # check sufficient number of non-N calls
    if site.nonMissing() < minCalls:
        return False
    ...
    if not minAlleles <= nAlleles <= maxAlleles:
        return False
    ...
    if nAlleles > 1:
        ...
        if (
            minVarCount
            and sorted(binBaseFreqs(numBases, asCounts=True))[-2] < minVarCount
        ):
            return False
        # check maximum heterozygots?
        if maxHet != None and site.hets(samples) > maxHet:
            return False
        # if there is a frequency cutoff
        if minFreq and not minFreq <= sorted(binBaseFreqs(numBases))[-2]:
            return False
        if maxFreq and not sorted(binBaseFreqs(numBases))[-2] <= maxFreq:
            return False
        # if checking HWE
        if HWE_P:
            ...
            elif not inHWE(site.asList(mode="diplo"), HWE_P, side=HWE_side):
                return False
    ...
    if len(popNames) >= 1:
        for popName in popNames:
            if minPopCalls:
                popCalls = sum(
                    [
                        site.genotypes[sample].isMissing() == False
                        for sample in site.pops[popName]
                    ]
                )
                if popCalls < minPopCalls[popName]:
                    return False
        ...
        if fixed or minPopAlleles or maxPopAlleles:
            ...
            if fixed and not (
                set([len(alleles) for alleles in allelesByPop]) == {1}
                and len(set([a for popAlleles in allelesByPop for a in popAlleles])) > 1
            ):
                return False
...
class Alignment:
    ...
    def __init__(
        ...
        assert (
            not sequences is numArray is length is None
        ), "Specify sequences or length of empty sequence object."
        if sequences is not None:
            assert isinstance(
                sequences, (list, tuple, np.ndarray)
            ), "Sequences must be a list, tuple or numpy array."
            if isinstance(sequences, np.ndarray):
                seqArray = sequences
            else:
                seqArray = np.array([list(seq) for seq in sequences])

            if numArray is not None:
                assert (
                    numArray.shape == sequences.shape
                ), "Numeric array is different shape from sequence array."
            else:
                numArray = seqArrayToNumArray(seqArray)

        elif numArray is not None:
            assert isinstance(
                numArray, np.ndarray
            ), "Numeric sequences must be a numpy array."
        ...
        if names is None:
            ...
            assert len(names) == self.N, "Incorrect number of names."
        self.names = np.array(names)

        if sampleNames is None:
            sampleNames = self.names
        else:
            assert len(sampleNames) == self.N, "Incorrect number of sample names."
        ...
        if groups is not None:
            ...
            for name in self.names:
                if name not in self.indGroupDict:
                    self.indGroupDict[name] = []
    ...
    def subset(self, indices=None, names=None, groups=None):
        ...
        return Alignment(
            sequences=self.array[indices],
            numArray=self.numArray[indices],
            names=self.names[indices],
            groups=self.groups[indices],
            sampleNames=self.sampleNames[indices],
        )

    def slice(self, indices=None, startPos=None, endPos=None):
        if indices is None:
            if startPos is None:
                startPos = min(self.positions)
            if endPos is None:
                endPos = max(self.positions)
            indices = [
                x for x in range(self.l) if startPos <= self.positions[x] <= endPos
            ]
    ...
    def numColumn(self, x):
        return self.numArray[:, x]
    ...
    def distMatrix(self, minSites=None):
        ...
        if minSites:
            pairNonNan = (
                self._pairNonNan_
                if self._pairNonNan_ is not None
                else self.pairNonNan()
            )
    ...
    def sampleHet(self, sampleNames=None, asList=False):
        if sampleNames is None:
            sampleNames, sampleIndices = uniqueIndices(
                self.sampleNames, preserveOrder=True
            )
        else:
            sampleIndices = [
                np.where(self.sampleNames == sampleName)[0]
                for sampleName in sampleNames
            ]
        # if a pre-computed distance matrix is available, use that
        if self._distMat_ is not None:
            hets = [
                self._distMat_[x[0], x[1]] if len(x) == 2 else np.NaN
    ...
    def groupDistStats(self, doPairs=True, minSites=None, minData=0.01):
        ...
        for x in range(nPops):
            output["pi_" + pops[x]] = nanmean_min(
                distMat[np.ix_(popIndices[x], popIndices[x])], min=minData
            )

        if nPops == 1 or not doPairs:
            return output
        ...
        for x in range(nPops - 1):
            for y in range(x + 1, nPops):
                # dxy
                output["dxy_" + pops[x] + "_" + pops[y]] = output[
                    "dxy_" + pops[y] + "_" + pops[x]
                ] = nanmean_min(
                    distMat[np.ix_(popIndices[x], popIndices[y])], min=minData
                )
                ...
                w = 1.0 * n_x / (n_x + n_y)
                pi_s = w * (output["pi_" + pops[x]]) + (1 - w) * (
                    output["pi_" + pops[y]]
                )
                pi_t = nanmean_min(
                    distMat[
                        np.ix_(
                            popIndices[x] + popIndices[y], popIndices[x] + popIndices[y]
                        )
                    ],
                    min=minData,
                )
                output["Fst_" + pops[x] + "_" + pops[y]] = output[
                    "Fst_" + pops[y] + "_" + pops[x]
                ] = (1 - pi_s / pi_t)
    ...
    def varSites(self, indices=None, names=None):
        if names is not None:
            indices = np.where(np.in1d(aln.names, names))[0]
        if indices is None:
            indices = np.arange(self.N)
        return np.where(
            [
                numVar(self.numArray[indices, x][self.nanMask[indices, x]])
    ...
    def groupFreqStats(self):
        ...
        for groupName in np.unique(self.groups):
            ...
            l = len(siteIdx)
            if l >= 1:
                siteBaseCounts = np.apply_along_axis(
                    binBaseFreqs,
                    0,
                    self.numArray[np.ix_(seqIdx, siteIdx)],
                    asCounts=True,
                )
                ...
                S = sum(sitePi != 0.0)
                thetaPi = sum(sitePi)
                thetaW = S / sum(1.0 / np.arange(1, N))
                TajD = TajimaD(N, S, thetaPi)
            else:
                S = thetaPi = thetaW = TajD = np.NaN
    ...
    def biSites(self):
        return np.where(
            [
                len(np.unique(self.numArray[:, x][self.nanMask[:, x]])) == 2
                for x in range(self.l)
            ]
        )[0]
    ...
    def siteNonNan(self, sites=None, prop=False):
        ...
        if prop:
            return np.mean(self.nanMask[:, sites], axis=0)
        return np.sum(self.nanMask[:, sites], axis=0)

    def seqNonNan(self, prop=False):
        if prop:
            return np.mean(self.nanMask, axis=1)
    ...
    def pairNonNan(self):
        ...
        for i in range(self.N - 1):
            for j in range(i + 1, self.N):
                self._pairNonNan_[i, j] = self._pairNonNan_[j, i] = np.sum(
                    self.nanMask[i, :] & self.nanMask[j, :]
                )
    ...
    def siteFreqs(self, sites=None, asCounts=False):
        ...
        return np.array(
            [
                binBaseFreqs(self.numArray[:, x][self.nanMask[:, x]], asCounts=asCounts)
                for x in sites
            ]
        )
    ...
    def consensus(self, minData=0.001):
        ...
        return [
            consensus(self.array[:, i][self.nanMask[:, i]])
            if propData[i] >= minData
            else "N"
    ...
    def sampleAlleles(self, sampleNames=None, asList=False, numeric=False):
        ...
        if numeric:
            sampleAlleles = [
                [
                    set(self.numArray[sidx, i][self.nanMask[sidx, i]])
                    for sidx in sampleIndices
                ]
                for i in range(self.l)
            ]
        else:
            sampleAlleles = [
                [
                    set(self.array[sidx, i][self.nanMask[sidx, i]])
                    for sidx in sampleIndices
                ]
                for i in range(self.l)
            ]
    ...
    def LDmatrix(self, stat="r2"):
        ...
        for x in range(self.l):
            for y in range(x, self.l):
                LDmat[x, y] = LDmat[y, x] = LD(
                    self.numArray[:, x], self.numArray[:, y]
                )[stat]
        return LDmat
...
def genoToAlignment(seqDict, sampleData=None, genoFormat="diplo", positions=None):
    ...
    for indName in seqDict.keys():
        ...
        ploidy = (
            ...
            if indName in sampleData.ploidy and sampleData.ploidy[indName] != None
            else len(seqList)
        )
        if ploidy is not None:
            assert len(seqList) == ploidy, (
                "Sample ploidy ("
                + str(ploidy)
                + ") doesn't match number of sequences ("
                + str(len(seqList))
                + ")"
            )
        if ploidy != 1:
            haploidSeqs += seqList
            seqNames += [
                indName + "_" + letter for letter in string.ascii_uppercase[:ploidy]
            ]
    ...
    return Alignment(
        sequences=[haploidSeqs[i] for i in order],
...
def LD(basesA, basesB, ancA=None, ancB=None):
    ...
    if not len(allelesA) == len(allelesB) == 2:
        return {"D": np.NaN, "Dprime": np.NaN, "r": np.NaN, "r2": np.NaN}

    if ancA is None:
        ancA = allelesA[countsA == max(countsA)][0]
    else:
        assert ancA in allelesA, "ancestral allele not present"

    if ancB is None:
        ancB = allelesB[countsB == max(countsB)][0]
    else:
        assert ancB in allelesB, "ancestral allele not present"
...
def maxLDphase(aln, sampleIndices=None, stat="r2"):
    ...
    assert aln.N == sum(
        [len(ind) for ind in sampleIndices]
    ), "Mistmatch between number of indices and sequences"
    assert len(aln.biSites()) == len(
        aln.varSites()
    ), "Only biallelic or invariant sites are permitted"

    nHets = np.array(
        [
            sum(
                [
                    len(np.unique(aln.numArray[ind, x][aln.nanMask[ind, x]])) > 1
                    for ind in sampleIndices
                ]
            )
            for x in range(aln.l)
        ]
    )
    ...
    if len(sitesToDo) >= 2:
        ...
        first = sitesToDo[0]
        newNumArray[:, first] = list(
            itertools.chain(*[sorted(newNumArray[ind, first]) for ind in sampleIndices])
        )
        newNumArray[:, first] = list(
            itertools.chain(*[sorted(newNumArray[ind, first]) for ind in sampleIndices])
        )
        ...
        for x in range(1, len(sitesToDo)):
            option1 = list(
                itertools.chain(
                    *[sorted(newNumArray[ind, sitesToDo[x]]) for ind in sampleIndices]
                )
            )
            option2 = list(
                itertools.chain(
                    *[
                        sorted(newNumArray[ind, sitesToDo[x]])[::-1]
                        for ind in sampleIndices
                    ]
                )
            )
            LD1 = np.mean(
                [LD(newNumArray[:, sitesToDo[y]], option1)[stat] for y in range(x)]
            )
            LD2 = np.mean(
                [LD(newNumArray[:, sitesToDo[y]], option2)[stat] for y in range(x)]
            )
    ...
    return Alignment(
        ...
        names=aln.names,
        groups=aln.groups,
        positions=aln.positions,
        sampleNames=aln.sampleNames,
    )
...
def distMatrix(sequences):
    ...
    for i in range(N - 1):
        for j in range(i + 1, N):
            ...
            distMat[i, j] = distMat[j, i] = numHamming(
                DNAarray[i, :][nanMask], DNAarray[j, :][nanMask]
            )
    return distMat
...
class SampleData:
    def __init__(
        ...
        assert (
            len(popNames) == len(popInds) == len(popNumbers)
        ), "Names, inds and numbers should be same length."
...
def popDiv(Aln, doPairs=True):
    ...
    # pi for each pop
    for x in range(nPops):
        output["pi_" + pops[x]] = np.nanmean(
            distMat[np.ix_(popIndices[x], popIndices[x])]
        )
    ...
    for x in range(nPops - 1):
        for y in range(x + 1, nPops):
            ...
            output["dxy_" + pops[x] + "_" + pops[y]] = output[
                "dxy_" + pops[y] + "_" + pops[x]
            ] = np.nanmean(distMat[np.ix_(popIndices[x], popIndices[y])])
            ...
            pi_t = np.nanmean(
                distMat[
                    np.ix_(popIndices[x] + popIndices[y], popIndices[x] + popIndices[y])
                ]
            )
            output["Fst_" + pops[x] + "_" + pops[y]] = output[
                "Fst_" + pops[y] + "_" + pops[x]
...
def fhom_old(p1, p2, p3, p4):
    return f4(p1, p2, p3, p4).sum() * 1.0 / f4(p1, p3, p3, p4).sum()
...
def fhom_new(p1, p2, p3, p4):
    ...
    for f4
    """
    return (f4_c(p1, p2, p3, p4)).sum() * 1.0 / (f4_c(p1, p3, p3, p4)).sum()
...
def D(p1, p2, p3, p4):
    return (
        f4(p1, p2, p3, p4).sum()
        * 1.0
        / ((1 - p1) * p2 * p3 * (1 - p4) + p1 * (1 - p2) * p3 * (1 - p4)).sum()
    )
...
def fd(p1, p2, p3, p4):
    ...
    return f4(p1, p2, p3, p4).sum() * 1.0 / f4(p1, pd, pd, p4).sum()
...
def fd_new(p1, p2, p3, p4):
    ...
    """
    pd = p2 * (p2 > p3) + p3 * (p3 >= p2)
    return (f4_c(p1, p2, p3, p4)).sum() * 1.0 / (f4_c(p1, pd, pd, p4)).sum()
...
def fdm(p1, p2, p3, p4):
    ...
    return f4(p1, p2, p3, p4).sum() * 1.0 / denom.sum()
...
def fdm_new(p1, p2, p3, p4):
    ...
    return (f4_c(p1, p2, p3, p4)).sum() * 1.0 / denom.sum()
...
def fh(p1, p2, p3, p4):
    ...
    return (
        (f4(p1, p2, p3, p4) + f4(1 - p1, 1 - p2, 1 - p3, 1 - p4)).sum()
        * 1.0
        / denom.sum()
    )
...
def fourPop(aln, P1, P2, P3, P4, minData, polarize=False, fixed=False):
    ...
    biallelic = [
        len(np.unique(all4Aln.numArray[:, x][all4Aln.nanMask[:, x]])) == 2
        for x in range(all4Aln.l)
    ]

    enoughData = (
        (P1Aln.siteNonNan() * 1.0 / P1Aln.N >= minData)
        & (P2Aln.siteNonNan() * 1.0 / P2Aln.N >= minData)
        & (P3Aln.siteNonNan() * 1.0 / P3Aln.N >= minData)
        & (P4Aln.siteNonNan() * 1.0 / P4Aln.N >= minData)
    )
    ...
    if len(goodSites) >= 1:
        if polarize:
            alleleIndex = np.where((all4freqs > 0) & (P4freqs == 0))
        elif fixed:
            alleleIndex = np.where(
                (all4freqs > 0)
                & (P4freqs == 0)
                & ((P1freqs == 0) | (P1freqs == 1))
                & ((P2freqs == 0) | (P2freqs == 1))
            ...
            alleleIndex = (
                np.arange(all4freqs.shape[0]),
                np.argsort(all4freqs, axis=1)[:, 2],
            )
...
def popSiteFreqs(aln, minData=0):
    ...
    if minData > 0:
        ...
        for x in range(len(pops)):
            _popSiteFreqs[x][~popDataMask[x], :] = np.array([np.nan] * 4)
...
class GenoWindow:
    ...
    def __init__(
        ...
        if sites is not None and positions is not None:
            if not len(sites) == len(positions) == 0:
                assert (
                    len(set([len(site) for site in sites])) == 1
                ), "Number of genotypes per site must be equal."
                assert len(sites[0]) == len(
                    names
                ), "Number of names must match number of genotypes per site."
                assert len(positions) == len(
                    sites
                ), "Positions must match number of sites"
        else:
            sites = []
            positions = []
    ...
    def addBlock(self, sites, positions):
        ...
        assert (
            len(sites[0]) == self.n
        ), "Number of genotypes per site must match number of names."
        assert len(positions) == len(sites), "Positions must match number of sites"
        assert np.all(
            self.limits[0] <= positions <= self.limits[1]
        ), "Position outside of window limit"
        self.sites += sites
        self.positions += positions
    ...
    def addSite(self, GTs, position=np.NaN, ignorePosition=False):
        ...
        if not ignorePosition:
            assert self.limits[0] <= position <= self.limits[1], (
                "Position: "
                + str(position)
                + " outside of window limits: "
                + "-".join([str(l) for l in self.limits])
            )
        else:
            position = np.NaN
    ...
    def slide(self, step=None, newLimits=None):
        ...
        if step:
            self.limits = [l + step for l in self.limits]
        else:
            self.limits = newLimits
        i = 0
        end = len(self.positions)
        while i < end and self.positions[i] < self.limits[0]:
            i += 1
...
def parseGenoLine(
    ...
    if line and line != "":
        ...
        if precompDict and GTstring in precompDict:
            ...
            GTs = GTstring.split()
            if splitPhased:
                GTs = [a for GT in GTs for a in list(GT)[::2]]
            if type != str:
        ...
        return {
            "scaffold": lineData[scafCol] if scafCol >= 0 else None,
...
class GenoFileReader:
    def __init__(
        ...
        ploidy=None,
        precomp=True,
        precompMaxSize=10000,
    ):
        self.genoFile = genoFile
        if not headerLine:
            headerLine = getNext(genoFile)
        ...
        if splitPhased:
            assert (
                ploidy is not None
            ), "Ploidy must be defined for splitting phased sequences"
            if self.names:
    ...
    def siteBySite(self, asDict=True):
        for line in self.genoFile:
            yield parseGenoLine(
                ...
                asDict,
                self.precompDict,
                addToPrecomp=self.precompDict["__counter__"]
                < self.precompDict["__maxSize__"],
            )

    def nextSite(self, asDict=True):
        try:
            line = getNext(self.genoFile)
        except:
            line = None
        return parseGenoLine(
            line,
            self.names,
            self.scafCol,
            self.posCol,
...
def parseGenoFile(
    ...
    # file reader
    reader = GenoFileReader(
        genoFile, headerLine, splitPhased=splitPhased, ploidy=ploidy
    )
    ...
    if names:
        ...
        if splitPhased:
            names = makeHaploidNames(names, ploidy)
    else:
    ...
    for siteData in reader.siteBySite(asDict=extractSpecificGTs):
        GTs = (
            [siteData["GTs"][name] for name in names]
            if extractSpecificGTs
            else siteData["GTs"]
        )
        window.addSite(
            GTs=GTs, position=siteData["position"], ignorePosition=not includePositions
        )
...
def slidingCoordWindows(
    ...
    while site["position"] is not None:
        # build window
        while (
            site["scaffold"] == window.scaffold and site["position"] <= window.limits[1]
        ):
            if site["position"] >= window.limits[0]:
                # add this site to the window
                GTs = (
                    [site["GTs"][name] for name in names]
                    if extractSpecificGTs
                    else site["GTs"]
                )
        ...
        # otherwise we're on a new scaffold (or its the end of the file)
            ...
            if (
                ...
                or (include and site["scaffold"] in include)
                or (exclude and site["scaffold"] not in exclude)
            ):
                window = GenoWindow(
                    scaffold=site["scaffold"],
                    limits=[1, windSize],
                    names=names,
                    ID=windowsDone + 1,
                )
            ...
            # if its a scaf we don't want, were going to read lines until we're on one we do want
                ...
                while (
                    site["scaffold"] == badScaf
                    or (
                        include
                        and site["scaffold"] not in include
                        and site["scaffold"] is not None
                    )
                    or (
                        exclude
                        and site["scaffold"] in exclude
                        and site["scaffold"] is not None
                    )
                ):
                    site = reader.nextSite(asDict=extractSpecificGTs)
...
def slidingSitesWindows(
    ...
    if not minSites:
        minSites = windSites  # if minSites < eindSites, windows at ends of scaffolds can still be emmitted
    # file reader
    reader = GenoFileReader(
        genoFile, headerLine, splitPhased=splitPhased, ploidy=ploidy
    )
    ...
    while site:
        # build window
        while (
            site["scaffold"] == window.scaffold
            and window.seqLen() < windSites
            and (
                window.seqLen() == 0 or site["position"] - window.firstPos() <= maxDist
            )
        ):
            # add this site to the window
            GTs = (
        ...
        if window.seqLen() >= minSites:
            ...
            # otherwise we're on a new scaffold (or its the end of the file)
                ...
                if (
                    ...
                    window = GenoWindow(
                        scaffold=site["scaffold"], names=names, ID=windowsDone + 1
                    )
...
def predefinedCoordWindows(
    ...
    for w in range(len(windCoords)):
        ...
        if window and window.scaffold == windCoords[w][0]:
            ...
            window = GenoWindow(
                scaffold=windCoords[w][0],
                limits=[windCoords[w][1], windCoords[w][2]],
                names=names,
                ID=ID,
            )
        ...
        while site["scaffold"] and (
            site["scaffold"] not in scafs or scafs.index(site["scaffold"]) < windScafIdx
        ):
            badScaf = site["scaffold"]
        ...
        while (
            site["scaffold"] == window.scaffold and site["position"] < window.limits[0]
        ):
            site = reader.nextSite(asDict=extractSpecificGTs)
        ...
        while (
            site["scaffold"] == window.scaffold
            and window.limits[0] <= site["position"] <= window.limits[1]
        ):
            # add this site to the window
            GTs = (
                [site["GTs"][name] for name in names]
...
# sliding window generator function
def nonOverlappingSitesWindows(
    genoFile,
    windSites,
    headerLine=None,
    names=None,
    ...
    while True:
        ...
        while (
            window
            and site["scaffold"] == window.scaffold
            and window.seqLen() < windSites
        ):
            # add this site to the window
            GTs = (
...
def makeAlnString(
    names=None, seqs=None, seqDict=None, outFormat="phylip", lineLen=None, NtoGap=False
):
    ...
    if NtoGap:
        seqs = [seq.translate(missingtrans) for seq in seqs]
    output = []
    nSamp = len(names)
...
def parsePhylip(string, asList=False):
    ...
    names = [
        [lineParts[headIdx[i] + 1 + j][0] for j in range(Ns[i])]
...
def makeDistMatPhylipString(distArray, names, roundTo=10):
    ...
    for i in range(len(names)):
        output += (
            str(names[i])
            + "  "
            + " ".join(distArray[i, :].round(roundTo).astype(str))
            + "\n"
        )
    return output
...
def parseRegionText(regionText):
    ...
    seqName = splitText[0]
    if len(splitText) < 3 or splitText[2] == "":
        ori = "+"
    else:
        ori = splitText[2]
    if ori not in "+-":
        raise ValueError("Incorrect region specification")
    try:
...
class Intervals:
    def __init__(
        self, regions=None, tuples=None, chroms=None, starts=None, ends=None, type=int
    ):
        ...
        if tuples is not None:
            ...
            self.starts = np.array(
                [t[1] if len(t) > 1 and t[1] is not None else 0 for t in tuples],
                dtype=type,
            )
            self.ends = np.array(
                [
            ...
            self.chroms = (
                np.array(chroms, dtype=str)
                if chroms is not None
                else np.repeat("", len(starts))
            )
            self.starts = (
                np.array(starts, dtype=type)
                if starts is not None
                else np.repeat(0, len(chroms))
            )
            self.ends = (
                np.array(ends, dtype=type)
                if ends is not None
                else np.array(starts, dtype=type)
                if starts is not None
                else np.repeat(np.inf, len(chroms))
            )
    ...
    def asRegionText(self):
        return [
            "{}{}{}{}{}".format(
                ...
                int(self.ends[i])
                if self.starts[i] > 0 and self.ends[i] < np.inf
                else "",
            )
    ...
    def reduced(self, skipSort=False):
        ...
        for i in range(1, self.l):
            ...
            if self.chroms[i] == newChroms[-1] and self.starts[i] <= newEnds[-1]:
                ...
                if self.ends[i] > newEnds[-1]:
                    newEnds[-1] = self.ends[i]
